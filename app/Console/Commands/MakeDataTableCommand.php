...
class MakeDataTableCommand extends GeneratorCommand
{
    ...
    protected function buildClass($name)
    {
        ...
        $this->replaceFilter($stub)
            ->replaceModelImport($stub)
             ->replaceModel($stub)
             ->replaceBuilder($stub)
             ->replaceColumns($stub)
             ->replaceButtons($stub)
    ...
    protected function replaceParam(&$stub)
    {

        $model = explode('\\', $this->getModel());
        $model = explode('\\', $this->getModel());
        $model = array_pop($model);
        $inLower = Str::of($model)->lower();
        $stub = str_replace(
            'DummyParam', $inLower, $stub
        );
    ...
    protected function replaceRoute(&$stub)
    {
        ...
        $model = explode('\\', $this->getModel());
        $model = array_pop($model);
        $inLower = Str::of($model)->lower()->plural();
        $stub = str_replace(
            'DummyRoute', $inLower, $stub
        );
    ...
    protected function replaceFilter(&$stub){
        $stub = str_replace('DummyFilters', $this->getFilters(), $stub);
        return $this;
    }

    protected function getFilters(){
        if ($this->option('columns') != '') {
            return $this->parseFilter($this->option('columns'));
        }
        else return '';
    }

    protected function replaceQuery(&$stub){
        $stub = str_replace('FilterQuery', $this->getFilterQuery(), $stub);
        return $this;
    }

    protected function getFilterQuery(){
        if ($this->option('columns') != '') {
            return $this->parseFilterQuery($this->option('columns'));
        }
        else return '';
    }
    ...
    protected function parseFilter($definition, $indentation = 12)
    {
        $columns = is_array($definition) ? $definition : explode(',', $definition);
        $stub    = '';
        foreach ($columns as $key => $column) {
            $index = $key+1;
            $column = Str::of($column)->headline();
            $stub .= "'$index'=>'$column',";
    ...
    protected function parseFilterQuery($definition, $indentation = 12)
    {
        $columns = is_array($definition) ? $definition : explode(',', $definition);
        $stub    = '';
        foreach ($columns as $key => $column) {
            $stub .= '->filterColumn('."'".$column."'".', function($query, $keyword) {
